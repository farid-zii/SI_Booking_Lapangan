                ...
                <table id="basic-datatables" class="display table table-bordered table-hover mt-2">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            {{-- <td>{{$item->idJadwal}} </td> --}}
                            <td class="text-center">
                               <p >{{\Carbon\Carbon::parse($item->jadwal->tanggal)->format('d-m-Y')}} </p>
                            ...
                            <td>@rp($item->sisa)</td>
                            <td><span class="p-2 rounded fw-bold text-light {{$item->status=='Booking' ? 'bg-success':'bg-success p-2 text-white bg-opacity-50';}}">{{$item->status}}</span></td>
                            <td>
                                <a href="/booking/{{$item->id}}/edit" class="btn btn-warning"><i
